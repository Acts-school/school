...
export default function EventListClient() {
  ...
  const { data } = useEventsList(listParams);

  const events: EventListItem[] = data?.data ?? [];
      ...
      >
        ...
        <td>
          <div className="flex items-center gap-2">
            {role === "admin" && (
              <>
                <FormContainerClient table="event" type="update" data={item} />
                <FormContainerClient table="event" type="delete" id={item.id} />
              </>
            )}
          </div>
        </td>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {role === "admin" && <FormContainerClient table="event" type="create" />}
          </div>
        </div>
      </div>
      {/* LIST */}
