...
const UserCard = async ({
  ...
  const useLightText = type === "teacher" || type === "parent";
  ...
  return (
    <div className="rounded-2xl odd:bg-lamaPurple even:bg-lamaYellow p-4 flex-1 min-w-[130px]">
      ...
      <h1
        className={`text-2xl font-semibold my-4 ${useLightText ? "text-white" : ""}`}
      >
        {data}
      </h1>
      <h2
        className={`capitalize text-sm font-medium ${useLightText ? "text-white" : "text-gray-500"}`}
      >
        {type}s
      </h2>
    </div>
