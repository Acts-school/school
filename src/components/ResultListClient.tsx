...
import { useResultsList, type ResultListItem } from "@/hooks/useResults";
...
export default function ResultListClient() {
  ...
  const { data } = useResultsList(listParams);

  const results: ResultListItem[] = data?.data ?? [];
  ...
  const renderRow = (item: ResultListItem) => {
    ...
    const scoreHighlightClass = item.isBelowPassing
    ...
    return (
      ...
      >
        ...
        <td className={`hidden md:table-cell ${scoreHighlightClass}`}>{item.score}</td>
        ...
        <td>
          <div className="flex items-center gap-2">
            {(role === "admin" || role === "teacher") && (
              <>
                <FormContainerClient table="result" type="update" data={item} />
                <FormContainerClient table="result" type="delete" id={item.id} />
              </>
            )}
          </div>
        </td>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {(role === "admin" || role === "teacher") && (
              <FormContainerClient table="result" type="create" />
            )}
          </div>
        </div>
      </div>
