...
import { CompactReceiptDocument } from "@/lib/pdf/ReceiptDocument";
...
export async function GET(
  ...
  try {
    ...
    const element = CompactReceiptDocument({
      schoolName,
      payment: {
        id: payment.id,
    ...
    return new NextResponse(body, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename=receipt-${payment.id}-compact.pdf`,
      },
    });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error("Error generating compact receipt PDF:", error);
